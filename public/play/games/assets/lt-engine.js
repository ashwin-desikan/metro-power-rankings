...
(function () {
  ...
  var table, QUESTIONS = [], i = 0, soundOn = true, locked = false, actx = null;
  ...
  function render() {
    locked = false; var s = QUESTIONS[i];
    ...
    order.forEach(function (idx) { var opt = s.opts[idx]; var btn = document.createElement("button"); btn.className = "opt"; btn.innerHTML = (opt.em ? '<span class="em">' + opt.em + '</span>' : '') + '<span>' + opt.t + '</span>'; btn.onclick = function () { choose(btn, idx === s.ans); }; o.appendChild(btn); });
    buildProgress();
  }
  function choose(btn, correct) {
    if (locked) return;
    if (correct) { locked = true; btn.classList.add("right"); document.querySelectorAll(".opt").forEach(function (b) { if (b !== btn) b.classList.add("dim"); }); $("toast").textContent = "🌟 Great detective work!"; cheer(); confetti(); var s = QUESTIONS[i]; if (s.view !== "bars" && s.hl) $("view").innerHTML = tableHTML(s.hl); $("fact").textContent = s.fact; if (s.link) { $("golink").style.display = "inline-block"; $("golink").href = s.link.url; $("golink").textContent = s.link.label; } else $("golink").style.display = "none"; $("reveal").className = "reveal show"; $("next").textContent = i === QUESTIONS.length - 1 ? "Finish →" : "Next clue →"; $("next").className = "next show"; }
    else { btn.classList.add("wrong"); $("toast").textContent = "Not quite — look again! 🔍"; nope(); setTimeout(function () { btn.classList.remove("wrong"); }, 450); }
  }
