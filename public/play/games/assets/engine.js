...
(function () {
  ...
  function finale() {
    ...
    ["place", "q", "read", "opts", "toast", "reveal", "next"].forEach(function (id) { var el = $(id); if (el) el.style.display = "none"; });
  ...
  function restore() {
    $("scene").style.display = "";
    ["place", "q", "read", "opts", "toast", "reveal", "next"].forEach(function (id) { var el = $(id); if (el) el.style.display = ""; });
  ...
  $("read").onclick = function () { var s = STOPS[i]; Scoring.readStarted(); speak(s.place + ". " + s.q, function () { Scoring.readEnded(); }); };
