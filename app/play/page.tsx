...
const GAMES: Game[] = [
  // ---- Learn & Play
  { title: "Penalty Shootout!", emoji: "\u{26BD}", file: "penalty-shootout.html", ages: "6–10", blurb: "Answer to take your shot, then score. 🔥 Sudden Death makes every shot a two-step question: club to city to country, flag to country to capital.", group: "learn", level: 2, topics: ["geography", "sports"] },
  ...
  { title: "Champions Duel", emoji: "\u{1F3C6}", file: "champions-duel.html", ages: "7–11", blurb: "459 real finals across EIGHT competitions. Tap the winner, then answer the harder half: which year, what score, by how many goals.", group: "learn", level: 3, topics: ["sports", "history-civics"] },
  ...
  { title: "Champions", emoji: "\u{1F3C5}", file: "champions.html", ages: "7–11", blurb: "Which sport is each nation best at? 🚀 Expert flips the question, and 🥇 Medal Table opens 142 Olympic nations: most golds, first Games, medal gaps.", group: "learn", level: 3, topics: ["sports", "geography"] },
  ...
  { title: "Who's the Boss?", emoji: "\u{1F3DB}\u{FE0F}", file: "whos-the-boss.html", ages: "7–11", blurb: "President, Prime Minister or King: whose job is it? Then 201 real world leaders, and the difference between running a country and representing it.", group: "civics", level: 3, topics: ["history-civics"] },
  ...
  { title: "US or UK?", emoji: "\u{1F5FD}", file: "us-or-uk.html", ages: "7–11", blurb: "98 things to sort: US, UK, or both. Plus 48 word pairs, because a boot is a trunk and chips are crisps.", group: "civics", level: 3, topics: ["history-civics"] },

  // ---- Count & Think
  { title: "Stadium Stacker", emoji: "\u{1F3DF}\u{FE0F}", file: "stadium-stacker.html", ages: "7–10", blurb: "Build match-day crowds from hundreds, tens and ones, then take the 🚀 Expert level: place value in the MILLIONS on real metro populations, order five cities, count through zero.", group: "think", level: 3, topics: ["maths", "sports"] },
  { title: "Big Match Adder", emoji: "➕", file: "big-match-adder.html", ages: "7–10", blurb: "Add and subtract crowds like a club accountant: estimate first, type the exact answer, prove it with the inverse check. 🚀 Expert brings five-digit crowds and missing-number puzzles.", group: "think", level: 3, topics: ["maths", "sports"] },
  { title: "Times-Table Striker", emoji: "✖\u{FE0F}", file: "times-table-striker.html", ages: "7–10", blurb: "Times tables through three-pointers, cricket fours and rowing eights. 🚀 Expert adds remainders, missing numbers and 2-digit×2-digit, and ⚡ Blitz gives you 60 seconds to set a record.", group: "think", level: 3, topics: ["maths", "sports"] },
  ...
  { title: "Rhythm Echo", emoji: "\u{1F941}", file: "rhythm-echo.html", ages: "6\u201310", blurb: "Listen to a rhythm, then tap it back. Real notation: a semibreve is 1, a minim \u00BD, a crotchet \u00BC, a quaver \u215B. Every rhythm is made fresh, so there is nothing to memorise, and tapping fast does not help.", group: "think", level: 2, topics: ["maths", "music"] },
  { title: "Note Value Lab", emoji: "\u{1F3BC}", file: "note-value-lab.html", ages: "7\u201310", blurb: "A bar holds exactly 4 beats. Drop notes in until it is full, and it plays. A semibreve is the whole bar, a minim \u00BD, a crotchet \u00BC, a quaver \u215B: fractions you can hear.", group: "think", level: 2, topics: ["maths", "music"] },
  ...
  { title: "Chart Champions", emoji: "\u{1F4CA}", file: "chart-champions.html", ages: "7–10", blurb: "Real league titles and skylines as bar charts, pictograms and tables. 🚀 Expert asks the harder question: is this chart being fair?", group: "think", level: 3, topics: ["maths", "sports"] },
  ...
  { title: "Call the Play", emoji: "\u{1F3C8}", file: "call-the-play.html", ages: "7\u201310", blurb: "You are the coach. Read the down, the yards to go, the clock and the score, then pick the play. Every call is graded best, reasonable or poor, because real football usually has more than one sensible option.", group: "coder", level: 3, topics: ["logic-coding", "sports"] },
  ...
  { title: "Champion Challenge", emoji: "\u{1F3C6}", file: "champion-challenge.html", ages: "7–10", blurb: "The decathlon: the hardest questions from across the arcade in one run, now including Riddle Stadium and Word Machine. 🚀 Expert lengthens it and draws only from the hard half.", group: "coder", level: 3, topics: ["logic-coding", "sports", "geography"] },

  // ---- Be the Ref
  { title: "Offside or Onside?", emoji: "\u{1F6A9}", file: "offside-or-onside.html", ages: "7–11", blurb: "You\u2019re the linesman: make the call, then name the rule that decided it. 10 cases including throw-ins, goal kicks and the second-last defender. \uD83D\uDD0D VAR review makes every margin a hair.", group: "rules", level: 3, topics: ["rules", "sports"] },
  { title: "How's That?", emoji: "\u{1F3CF}", file: "hows-that.html", ages: "7–11", blurb: "HOWZAT?! Call it, then name the law behind it. Bowled, caught behind, LBW, inside edge, no-ball and stumped. \uD83C\uDFA5 Third umpire adds umpire\u2019s call.", group: "rules", level: 3, topics: ["rules", "sports"] },
  { title: "Ball or Strike?", emoji: "\u{26BE}", file: "ball-or-strike.html", ages: "7–11", blurb: "Call the zone, then name the rule. The zone is sized to the batter, so the same pitch is a strike to one and a ball to another. \uD83D\uDD0D Replay booth shaves it to a unit.", group: "rules", level: 3, topics: ["rules", "sports"] },
  { title: "Catch or No Catch?", emoji: "\u{1F3C8}", file: "catch-or-no-catch.html", ages: "7–11", blurb: "Rule it, then say which part of the rule decided it. Toe-taps, the line, control, surviving the ground, the football move, and where college and the NFL disagree.", group: "rules", level: 3, topics: ["rules", "sports"] },
