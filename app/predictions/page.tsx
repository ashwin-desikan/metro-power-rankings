...
const LEAGUE_HUBS: { key: string; emoji: string; name: string; season: string; blurb: string; href?: string; game?: boolean }[] = [
  ...
  { key: "ucl", emoji: "\u{1F3C6}", name: "Champions League", season: "2026-27 season", blurb: "Champion, top-eight and knockout odds for all 36 clubs from the drawn league phase to the final, fitted on three decades of European match data and backtested against every real champion since 2004.", href: "/predictions/ucl", game: false },
];
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <PredHeader
        emoji="🔮"
        title="Predictions"
        sub="Forecasts across politics and sport, stated honestly: weighted averages, ranges from thousands
      ...
      <section id="elections" className="mb-10 rounded-2xl border p-5 sm:p-6" style={{ borderColor: "var(--border)" }}>
        ...
        <p className="text-sm text-[var(--text-muted)] max-w-3xl mb-2">
          The 2026 US midterms, the next UK election, and 2026 votes in Brazil, Israel and New Zealand.
        </p>
        <details className="max-w-3xl mb-4">
          <summary className="text-xs text-[var(--text-dim)] cursor-pointer hover:text-[var(--accent)]">More</summary>
          <div className="mt-2 text-sm text-[var(--text-muted)]">
            Covers the House, Senate and governors within the midterms, plus an early read on France 2027.
          </div>
        </details>
        {us && (
        ...
        <Link href="/elections/forecast" className="inline-flex items-center min-h-11 gap-1.5 text-sm font-semibold" style={{ color: "var(--accent)" }}>
          Full election forecasts <span aria-hidden>&rarr;</span>
        </Link>
      </section>

      {/* The Ledger - the accountability surface for everything else on this page */}
      <section id="ledger" className="mb-10 rounded-2xl border p-5 sm:p-6" style={{ borderColor: "var(--border)" }}>
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <h2 className="text-2xl font-bold flex items-center gap-2"><span aria-hidden>&#128211;</span> The Ledger</h2>
          <LiveDot />
        </div>
        <p className="text-sm text-[var(--text-muted)] max-w-3xl mb-2">
          Every forecast scored against the price the market closed at.
        </p>
        <details className="max-w-3xl mb-4">
          <summary className="text-xs text-[var(--text-dim)] cursor-pointer hover:text-[var(--accent)]">What this shows</summary>
          <div className="mt-2 text-sm text-[var(--text-muted)]">
            Anyone can publish a probability, so this is the record: the probability bins showing whether
            seventy per cent has meant seventy per cent, and the seasons the model beat the market named
            one by one. Over twenty thousand priced games, the market is still ahead.
          </div>
        </details>
        <Link href="/predictions/scoreboard" className="inline-flex items-center min-h-11 gap-1.5 text-sm font-semibold" style={{ color: "var(--accent)" }}>
          See how the models have done <span aria-hidden>&rarr;</span>
        </Link>
      </section>

      {/* League prediction hubs */}
      <section id="hubs" className="mb-10">
        <div className="flex items-baseline justify-between gap-3 mb-1 flex-wrap">
          <h2 className="text-2xl font-bold flex items-center gap-2"><span aria-hidden>&#127942;</span> League Prediction Hubs</h2>
        </div>
        <p className="text-sm text-[var(--text-muted)] max-w-3xl mb-2">
          Each hub runs tens of thousands of simulated seasons and publishes every team&apos;s odds at each stage.
        </p>
        <details className="max-w-3xl mb-5">
          <summary className="text-xs text-[var(--text-dim)] cursor-pointer hover:text-[var(--accent)]">More</summary>
          <div className="mt-2 text-sm text-[var(--text-muted)]">
            Most hubs include a Beat-the-Model card, exactly like the 2026 World Cup simulator below. The
            NFL, MLB, College Football and the Premier League are live; the Champions League arrives with
            the late-August draw.
          </div>
        </details>
      ...
      <section id="wc2026" className="mb-10 rounded-2xl border p-5 sm:p-6" style={{ borderColor: "var(--border)" }}>
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <h2 className="text-2xl font-bold flex items-center gap-2"><span aria-hidden>&#9917;</span> The template: World Cup 2026</h2>
          <LiveDot />
        </div>
        <p className="text-sm text-[var(--text-muted)] max-w-3xl mb-2">
          The working model the league hubs are built from.
        </p>
        <details className="max-w-3xl mb-4">
          <summary className="text-xs text-[var(--text-dim)] cursor-pointer hover:text-[var(--accent)]">More</summary>
          <div className="mt-2 text-sm text-[var(--text-muted)]">
            Title odds for every nation, updated as the tournament plays out, and a Beat-the-Model card
            where you back the calls the simulator is least sure about.
          </div>
        </details>
        <div className="flex flex-wrap gap-3">
          <Link href="/teams/national#wc2026" className="inline-flex items-center min-h-11 gap-1.5 rounded-lg font-semibold text-sm px-4 py-2" style={{ backgroundColor: "var(--accent)", color: "#08080D" }}>
            The World Cup simulator <span aria-hidden>&rarr;</span>
          </Link>
          <Link href="/play/picks" className="inline-flex items-center min-h-11 gap-1.5 rounded-lg border font-semibold text-sm px-4 py-2 hover:border-[var(--accent)] transition-colors" style={{ borderColor: "var(--border)", color: "var(--text)" }}>
      ...
      <SourcesCard>
        <p>
          Election numbers are this site&apos;s own aggregation model, rebuilt as new polls and fundamentals
          land. Every league hub below runs its own Monte Carlo simulation from that league&apos;s real
          remaining schedule, and states its own sim count, as-of date and sources on its own page. The
          Ledger scores every graded forecast on this page against the betting market that priced the same
          event - it is the one place all of them are held to the same standard.
        </p>
      </SourcesCard>
    </main>
  );
