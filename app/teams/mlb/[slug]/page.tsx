...
export default async function FranchisePage({ params }: Props) {
  ...
  // TopGamesTable). colMax is computed once over the full set.
  const topGamesColMax = Math.max(...topGamesWithOppSlug.map((g) => g.game_score), 0.0001);
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      >
        ...
        <ChampionBadge items={getCurrentChampionships(f.display_name, "Baseball")} />
          ...
          <HeartbreakTag league="mlb" slug={f.slug} className="mt-2" />
      ...
      <HeartbreakPanel league="mlb" slug={f.slug} className="mb-8" />
                ...
                {topGamesWithOppSlug.map((g, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <DataBar v={g.game_score} max={topGamesColMax} dp={3} color="var(--seq-4)" width={92} label="game score" />
                    </td>
                    </tr>
                  );
                })}
