...
def tournament_label_for(year, category):
...
def build_teams_index(totals_rows, summary_rows, federation_lookup):
    """Produce one team per Cur. Name. Totals rows are the primary source;
    teams that appear only in Int Summary (Cur. Name not in Totals) get a
    derived minimal profile."""
    by_cur_name = {}
...
def build_appearances(summary_rows, slug_for_cur_name):
    """{slug: [appearance rows...]}. One row per (Team, Year) entry. Includes
    the 'as' attribution when Team != Cur. Name (e.g., 'as Soviet Union')."""
    ...
    for r in summary_rows:
        ...
        out[slug].append({
            ...
            "category": cat,
            "tournament_label": tournament_label_for(r["year"], cat),
            "round_reached": deepest_round_label(r),
            "champion": r["champions"] == "Y",
            "team_as": r["team"] if r["team"] != r["cur_name"] else None,
            "group": r["group"],
        })
    # Sort newest first within each team.
    for slug in out:
        out[slug].sort(key=lambda x: -(x["year"] or 0))
    return dict(out)


def build_finals(finals_rows, slug_for_cur_name):
    """{slug: [final-match rows...]}. Newest first."""
    out = defaultdict(list)
    for r in finals_rows:
        slug = slug_for_cur_name.get(r["cur_name"])
        if not slug:
            continue
        opp_slug = slug_for_cur_name.get(r["opp_cur_name"]) if r["opp_cur_name"] else None
        category = (
        ...
        out[slug].append({
            ...
            "team_as": r["team_as"] if r["team_as"] != r["cur_name"] else None,
            "stadium": r["stadium"],
...
def build_tournament_hubs(summary_rows, slug_for_cur_name):
    ...
    for hub in TOURNAMENT_HUBS:
        cat = hub["category"]
        matching = [r for r in summary_rows if hub["filter"](r)]
        if not matching:
            continue
...
def main():
    ...
    appearances = build_appearances(summary_rows, slug_for_cur_name)
    print(f"  {sum(len(v) for v in appearances.values())} appearance rows across {len(appearances)} teams")

    print("\nBuilding finals...")
    finals_by_slug = build_finals(finals_rows, slug_for_cur_name)
