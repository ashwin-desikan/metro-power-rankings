...
def resolve_metro(slug_guess, country_slug=None, state_slug=None, text_fallback=""):
...
for slug,ld in current.items():
    ...
    if not cs: continue
    mn,msl=resolve_metro(CAPITAL_METRO_SLUG.get(slug), country_slug=slug, text_fallback=cname.get(slug,slug))
    ov=inp.get("monarch_overrides",{}).get(slug)
    _nm=ov["name"] if ov else ld["name"]
    _rl=ov["role"] if ov else ld.get("role","")
    add(_nm, f'{_rl}, {cname.get(slug,slug)}',"National",cname.get(slug,slug),cs,regime(slug)*GEO.get(slug,1.0),metro=mn,metro_slug=msl)

US=cscore["united-states"]
ex=cong=rj("public/data/us-congress.json")
usmn,usmsl=resolve_metro("washington-baltimore")
ex=cong["executive"]
...
for slug,sc in sscore.items():
    mn,msl=resolve_metro(STATE_METRO_SLUG.get(slug), state_slug=slug, text_fallback=sname.get(slug,slug))
    if slug in sleaders:
...
for ms,m in mayors.items():
    mn,msl=resolve_metro(ms, text_fallback=m.get("city",ms))
    add(m["mayor"], f'Mayor of {m.get("city",ms)}',"Mayor",m.get("city",ms),mscore.get(ms,0),W["mayor"],metro=mn,metro_slug=msl)

def memsum(org): return sum(cscore.get(s,0) for s,o in rj("public/data/country-orgs.json").items() if o.get(org)=="Member")
...
for org in ("NATO","UN","EU"):
    c=orgL[org]["current"]; mn,msl=resolve_metro(ORG_METRO_SLUG.get(org)); add(c["name"], f'{org} {c["role"]}',"Org",org,memsum(org),ORGW.get(org,W["org"]),metro=mn,metro_slug=msl)
for k,role in (("IMF","IMF Managing Director"),("WORLD_BANK","World Bank President"),("WTO","WTO Director-General")):
    mn,msl=resolve_metro(ORG_METRO_SLUG.get(k)); j="World Bank" if k=="WORLD_BANK" else k
    add(ORGH[k]["name"],role,"Org",j,world,ORGW[k],metro=mn,metro_slug=msl)
mn,msl=resolve_metro("frankfurt")
...
for k,info in CB.items():
    mn,msl=resolve_metro(CB_METRO_SLUG.get(k), country_slug=CBJ[k]); add(info["name"],info["title"],"Central bank",cname.get(CBJ[k]),cscore.get(CBJ[k],0),CBW.get(k,W["central_bank"]),metro=mn,metro_slug=msl)

bz=rj("public/data/billionaires.json")["billionaires"]
for b in bz[:60]:
    nb=(b.get("networth") or 0)/1000.0
    mn,msl=resolve_metro(BILLIONAIRE_METRO_SLUG.get(_bare(b["name"])), text_fallback=b.get("countryName"))
...
for k,info in inp.get("finance",{}).items():
    ...
    mn,msl=resolve_metro(info.get("metro_slug"))
...
for k,info in inp.get("judiciary",{}).items():
    ...
    mn,msl=resolve_metro(info.get("metro_slug"), country_slug=csl)
...
for blk,cat,fac in (("media","Media","media_factor"),("sport_governance","Sport","sport_factor"),("culture","Culture","culture_factor")):
    for k,info in inp.get(blk,{}).items():
        ...
        mn,msl=resolve_metro(info.get("metro_slug"), text_fallback=info.get("hq",""))
...
for k,info in inp.get("crown",{}).items():
    mn,msl=resolve_metro(info.get("metro_slug"), text_fallback=info.get("country",""))
...
for k,info in inp.get("faith",{}).items():
    mn,msl=resolve_metro(info.get("metro_slug"), text_fallback=info.get("seat",""))
