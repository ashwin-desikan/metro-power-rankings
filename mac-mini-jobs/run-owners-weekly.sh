...
STEP 0 -- sync and read. Run git pull --ff-only (abort and report if it fails). Read scripts/data/team-owners-seed.json (curated source of truth; rows with \"confidence\":\"contested\" are the live watchlist, each with pending_review_by and pending_when), scripts/data/owner-registry.json (canonical multi-club owner/entity registry, added 2026-09-24 -- see its _readme field and lib/ownerRegistry.ts for how it fixes cases like Wes Edens/V Sports at Aston Villa + Milwaukee Bucks, previously two unlinked owner_keys for the same person), and the deploy-discipline section of CLAUDE.md. The seed also covers the WNBA/NWSL/European-football/Brazilian rows added 2026-09-24 alongside scripts/fans/valuations_extended.csv -- treat them like any other seed row.

STEP 1 -- research, three passes. (a) WATCHLIST: for each contested row, WebSearch whether the deal RESOLVED (league vote passed, sale closed, deal collapsed) or materially moved. Real sources only: league releases, ESPN, Sportico, The Athletic, BBC, Forbes, AP, club statements. Resolution requires a reported league approval or completed closing; 'nears/agrees/expected' is movement. (b) SWEEP: search for NEW ownership news from the past 7-10 days the board does not carry, across Premier League and other top European football, NFL, NBA, MLB, NHL, MLS, WNBA, NWSL, F1, and other leagues present in the seed. Compare every hit against the seed before acting. (c) REGISTRY: for any change found in (a) or (b), check scripts/data/owner-registry.json for an existing entity covering that person/company. If one exists and the deal adds or drops a club, update that entity's clubs array (role control or minority) in the SAME commit as the seed change. If a finding reveals a NEW multi-club link the registry does not carry yet (same owner_key, or same real person/company under two different owner_keys/display names across two seed rows), add a new registry entity with id, displayName, aliasNames, aliasOwnerKeys (owner_key strings this entity controls under, CONTROL rows only), clubs (every club with a role), note. Never invent a link you have not verified this week; leave it for a human pass if uncertain. (d) LEAD SOURCE: sportsinvestingmarketmap.com is a weekly lead source, not a citable source. Open https://sportsinvestingmarketmap.com/signals and its category pages, and read only the signals dated since this job's last run (check $LOGDIR for the previous run's date if unsure). For each signal about team or league ownership, stakes, sales, or fund rules, open the PRIMARY source it links to (the club, league, regulator, or the news outlet actually reporting the deal) and verify the claim there before doing anything else with it. Treat an unverifiable signal as noise and drop it. Once verified against its primary source, fold the finding into the same STEP 2 apply pass as (a)/(b)/(c): propose or apply the change under this job's normal apply rules, citing the primary source you verified against in source_url and note. Never copy sportsinvestingmarketmap.com's wording into seed prose, and never cite sportsinvestingmarketmap.com itself as source_url or in a note -- it is a lead to chase, not evidence.

STEP 2 -- apply. Edit scripts/data/team-owners-seed.json (and scripts/data/owner-registry.json per STEP 1c) only for findings that are multi-source or primary-source; keep prose factual, attribute single-source claims in the note, never fabricate, no em dashes in prose. Control stays with the current owner until a league approval or closing is actually reported. Then run python3 scripts/build-team-owners-data.py --self-test, then python3 -c \"import json; json.load(open('scripts/data/owner-registry.json'))\" to confirm the registry is still valid JSON, then python3 scripts/build-team-owners-data.py (which rewrites $OWNERS_JSON), then python3 scripts/check-owners-watchlist.py; all must pass or do not commit (reset the tree and report the failure instead).

STEP 3 -- commit. The site reads $OWNERS_JSON at RUNTIME, so this commit needs NO Vercel build: the subject line MUST contain [vercel skip]. Stage ONLY scripts/data/team-owners-seed.json, scripts/data/owner-registry.json and $OWNERS_JSON. Do NOT edit lib/releases.ts or any file under app/ or lib/: any of those would need a paid build, and this job must never cause one.
