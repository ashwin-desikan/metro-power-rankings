...
> **⚠️ ARCHIVED HANDOFF. DO NOT ACTION. Superseded on its central claim, corrected 2026-08-07.**
> The paragraph below says `public/data/f1/data.json` "stays owned by the `f1-refresh.yml` GitHub Action". **That has inverted.** Both of that workflow's crons are commented out, and `mac-mini-jobs/run-f1-weekly.sh:10` states "Mini owns public/data/f1/data.json", builds it via `build-f1-data.py` and commits it. **Re-enabling `f1-refresh.yml` would create a duplicate-commit race on a single file.** The "Sunday 21:30 + Monday 14:00 local" schedule advice was also superseded by an hourly round-gated `StartInterval` poller (`com.citizenofnowhere.f1-weekly.plist`). The open question in this handoff is closed. Current mini schedule: `mac-mini-jobs/jobs.toml`.
> This file is a near-duplicate of `mini-migration-analysis.md`, which carries the same claim and fuller reasoning; keeping both means maintaining two copies of one superseded statement. Deletion candidate.
